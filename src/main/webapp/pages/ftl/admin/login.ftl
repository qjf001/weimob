...
<head>
    ...
    <link rel="shortcut icon" href="${rct.contextPath}/assets/images/dd.ico" type="image/x-icon">
    <title>xxxx系统</title>
    <script src="${rct.contextPath}/js/jquery-3.1.1.min.js"></script>
...
<div class="wrapper">
    ...
    <div class="container">
        ...
        <form id="loginForm" action="${rct.contextPath}/login" method="post">
            <input class="txt-input txtpd" name="username" id="username" lay-verify="required" type="text" value=""
                   placeholder="请输入账号" onkeydown="nameEnterKeyEvent();"/>
            <input class="txt-input txtpd" name="password" id="password" lay-verify="required" value=""
                   placeholder="请输入密码"
                   type="password" onkeydown="passEnterKeyEvent();"/>
            <div>
                <input class="txt-input txtpd" name="captcha" id="captcha" lay-verify="required" value="" placeholder="请输入验证码" type="text" onkeydown="codeEnterKeyEvent();" style="width: 160px;display: inline-block"/>
                <img src="${rct.contextPath}/captcha/shiro/get" style="display: inline-block;height: 40px;" onclick="codeReload($(this));"/>
            </div>
            <button id="login" type="button" onclick="page_save_ajax();">登录</button>
        </form>
...
<script type="text/javascript" src="${rct.contextPath}/js/jQuery.md5.js"></script>
<link rel="stylesheet" href="${rct.contextPath}/layui/css/layui.css"/>
<script src="${rct.contextPath}/layui/layui.js"></script>
<script type="text/javascript">

    layui.config({
        dir: '${rct.contextPath}/layui/' //layui.js 所在路径（注意，如果是script单独引入layui.js，无需设定该参数。），一般情况下可以无视
    ...
    var page_save_ajax = function () {
        ...
        if (!userName) {
            layer.alert("账号不能为空", {icon: 5, title: "账号"});
            return;
        }
        if (!password) {
            layer.alert("密码不能为空", {icon: 5, title: "密码"});
            return;
        }
        if (!captcha) {
            layer.alert("验证码不能为空", {icon: 5, title: "验证码"});
            return;
        }
    ...
    var codeReload = function(obj){
        $(obj).attr("src","${rct.contextPath}/captcha/shiro/get?time="+new Date().getTime());
        $('#captcha').val("");
    }
